...
# --------- Optuna class ---------#
class Optuna:
    """Simplified hyperparameter tuning using Optuna"""
    ...
    def sample_params(self, trial: optuna.Trial) -> Dict[str, Any]:
        ...
        if agent == 'ppo':
            # Sample architecture first (stored internally for Optuna)
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        try:
            ...
            # Create and train model with reduced verbosity
            agent.model = agent._create_model(trial_config['training'])
            agent.model.set_logger(None)  # Suppress training output
